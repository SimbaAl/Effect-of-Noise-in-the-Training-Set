...
ERR_DPA_TA_LSTM = Err_scheme_LSTM ./ (testing_samples * Phf); 
BER_DPA_TA_LSTM = Ber_scheme_LSTM/ (testing_samples * nSym * 48 * nBitPerSym);

%% Save the data in a .mat file
save('LSTM_DPA_TA.mat', 'Phf', 'BER_DPA_TA_LSTM', 'ERR_DPA_TA_LSTM');
...
figure;
plot(EbN0dB, ERR_DPA_TA_LSTM, 'r-s');
xlabel('SNR (dB)');
ylabel('Normalized Mean Squared Error (NMSE)');
title('Normalized MSE of Predicted Channel vs True Channel (NMSE\_scheme\_DNN) vs SNR');
grid on;

% Plot BER_scheme_DNN
figure;
plot(EbN0dB, BER_DPA_TA_LSTM, 'g-d');
xlabel('SNR (dB)');
ylabel('Bit Error Rate (BER)');
title('Bit Error Rate (BER\_scheme\LSTM) vs SNR');
grid on;
