...
class DNN(nn.Module):
    def __init__(self, input_size, hidden_layer1, hidden_layer2, hidden_layer3, output_size):
        ...
        nn.init.trunc_normal_(self.layer4.weight, mean=0.0, std=0.05)
...
if configuration_mode == 13:
    # [Code to load and preprocess data remains similar]
    ...
    model = DNN(int(dnn_input), int(hidden_layer1), int(hidden_layer2), int(hidden_layer3), int(dnn_output))
    ...
    for epoch in range(int(epoch)):
        ...
        print(f"Epoch {epoch}, Training Loss: {avg_loss}, Validation Loss: {val_loss}")
